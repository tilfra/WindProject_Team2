...
%% ---------------- Load wind speeds (2021) ----------------
T = readtable('Windspeeds.csv','PreserveVariableNames',true,'Delimiter',';');
timeRaw = T{:,1};
try
...
maskYear = (year(timeVec) == 2021);
timeVec = timeVec(maskYear);
u_col = u_all(maskYear);

H = numel(u_col);
fprintf('Loaded %d hourly wind speeds (year 2021)\n', H);
...
xy_arced = create_arced_layout(nT, spacing, theta_deg_arc, mean_dir, [0 0]);
